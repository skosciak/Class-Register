...
export function searchForMatch(database: string, data_object: object, data_to_search: object) {
    let match: number = 0;
    const perfect_match_array: {id: string, data: object}[] = [];
    const imperfect_match_array: {id: string, data: object}[] = [];
    let array_present: {array: boolean, length?: number};
    ...
    for (const data_key in data_object) {
        if (Object.keys(data_object[data_key]).length >= Object.keys(data_to_search).length){
            ...
            if (database === 'teachers' && array_present.array && Array.isArray(data_object[data_key]['subject'])){
                const temp_match: number = match;
                match = 0;
                data_object[data_key]['subject'].forEach((el: string) => {
...
export function returnFirstFreeID(database: string) {
